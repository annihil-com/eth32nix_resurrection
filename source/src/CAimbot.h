...
typedef enum {
	...
	HEAD_STATIC_SURFACE,			// generate static points  
	HEAD_XTRACE,
	HEAD_MAX
} headBoxtrace_t;
...
typedef enum {
	...
	BODY_STATIC_SURFACE,			// generate static points  
	BODY_XTRACE,
	BODY_MAX
} bodyBoxtrace_t;
...
public:
	...
	bool IsAimableWeapon(int weapon);
...
private:
	...
	void AdjustDelay(player_t *player);
	void AdjustHitboxes(player_t *player, hitbox_t 	*hbox);
	bool IsSingleShotWeapon(int weapon);
	void ShowAimpoints(trace_point *p, int maxTraces, vec3_t trOrigin, int skipEnt);
	void CalcTrPointsDist(vec3_t boxOrigin, trace_point *p, int maxTraces);
	...
	int lastShotTime;		// Time since last bullet was shot
