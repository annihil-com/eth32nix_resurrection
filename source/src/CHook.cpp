...
void CHook::GetPredictedPlayerState()
{
	...
	while(found) {
		found = memmem((void*)(found+1), 0x989680, &eth32.cg.snap->ps.origin, 0xa8);
		if (!found)
			return;
...
void CHook::InitPB()
{
	...
	(
		(void*)(eth32.pb.text+(uintptr_t)eth32.pb.module),
		eth32.pb.text_size,
	...
	eth32.pb.cvarlist = (pb_cvar_t **)*(uintptr_t *)(pos+6);
	eth32.pb.nCvars = (uintptr_t *)(eth32.pb.cvarlist+4);

	//verify
	uintptr_t r = (uintptr_t)eth32.pb.nCvars-(uintptr_t)eth32.pb.module;
...
{
	uintptr_t base = (uintptr_t)mbase;
	#define SYMBOL(x) (dlsym(handle, x) ? (uintptr_t)dlsym(handle, x)-base : 0)
...
void CHook::hookEtpro(bool state)
{
	...
	if (state){
		...
		// etpro guid
		orig_etproAntiCheat = (etproAntiCheat_t)Detour.CreateDetour((void *)(eth32.cg.module+0xa360), (void *)&eth32_etproAntiCheat);
		*((unsigned char *)(orig_etproAntiCheat + 4)) = 0xbb;	// movl imm32, %ebx
		*((void **)(orig_etproAntiCheat + 5)) = (void *)((uintptr_t)eth32.cg.module + 0xa369);	// lame but neccessary

		// etpro cvars proof
		mprotect(GET_PAGE(eth32.cg.module + 0xf718), 4096, PROT_READ | PROT_WRITE | PROT_EXEC);
		memcpy((void *)(eth32.cg.module + 0xf718), "\x31", 1);	// xorl %edx, %edx
		mprotect(GET_PAGE(eth32.cg.module + 0xf718), 4096, PROT_READ | PROT_EXEC);

	} else {
		Detour.RemoveDetour((void *)(eth32.cg.module+0xa360), (void *)orig_etproAntiCheat);
...
void CHook::hookCgame(bool state)
{
	...
	if (state && !hooked)
	{
		...
		orig_CG_Trace = (CG_Trace_t)(eth32.cg.module + eth32.cgMod->CG_Trace);

		if(eth32.cgMod->CG_AllocLocalEntity) orig_CG_AllocLocalEntity = (CG_AllocLocalEntity_t) (eth32.cg.module + eth32.cgMod->CG_AllocLocalEntity);
		if(eth32.cgMod->BG_FindClipForWeapon) orig_BG_FindClipForWeapon = (BG_FindClipForWeapon_t) (eth32.cg.module + eth32.cgMod->BG_FindClipForWeapon);
		if(eth32.cgMod->BG_FindAmmoForWeapon) orig_BG_FindAmmoForWeapon = (BG_FindAmmoForWeapon_t) (eth32.cg.module + eth32.cgMod->BG_FindAmmoForWeapon);
		if(eth32.cgMod->predictedPS) eth32.cg.predictedps = (playerState_t *) (eth32.cg.module + eth32.cgMod->predictedPS);
...
{
	uintptr_t dwBase	= (uintptr_t)eth32.cg.module;
	uintptr_t dwCent	= dwBase + eth32.cgMod->centBase;
	uintptr_t dwCi		= dwBase + eth32.cgMod->ciBase;
	...
	dwBase	= (uintptr_t)eth32.cg.module;//
	...
	if (enabled) {
		...
		*(char *)eth32.et->Con_DrawConsole = '\xc3';			//ret
		...
		if (eth32.cgameLoaded){
			...
			while ((uintptr_t)le <= (uintptr_t)eth32.cg.highle) {
				le->endTime = 0;
				le = (localEntity_t *) ((uintptr_t)le + sizeof(localEntity_t));
			}
...
void CHook::ModifyCgameCommands()
{
	...
	consoleCommand_t *cc = (consoleCommand_t*)((uintptr_t)eth32.cg.module + eth32.cgMod->commands);
...
int CDECL eth32_CG_syscall(int command, ...)
{
	...
	switch (command)
	{
		...
		case CG_PRINT: {
			...
			// Auto "vote no" kicks against you
			if (eth32.settings.autoVote) {
				char voteString[256];
				sprintf(voteString, "%s^7 called a vote.  Voting for: ", eth32.cg.players[eth32.cg.clientNum].name);

				if (strstr(analise_str, voteString)) { // Do nothing if we called the vote (to avoid canceling it).
					break;
				} else {	// If we aren't the caller then vote no.				
					if (strstr(analise_str, "called a vote.  Voting for: KICK")) {
		...
		case CG_TRANSLATE_STRING: 
		{
			...
			// to check if it's safe to vote so we don't get kicked for excess of flood, 1000ms is enough
			if (eth32.settings.autoComplaint) {
				if (eth32.cg.cgTime - Hook.lastVoteTime < 1000)
					break;

				if (strstr(analise_str, "File complaint against ")) {
					Syscall.CG_SendConsoleCommand("vote yes\n");
					Hook.lastVoteTime = eth32.cg.cgTime;
					break;
				}
			}
		...
		case CG_GETSERVERCOMMAND:
		{
			...
			if (eth32.cgMod->type != MOD_ETPRO && eth32.settings.autoPredictBots) {	
				if ( !strcmp( cmd, "sc0" ) ) {
					Engine.ParseScore(TEAM_AXIS);
					return ret;
				} else if ( !strcmp( cmd, "sc1" ) ) {
					Engine.ParseScore(TEAM_ALLIES);
					return ret;
				}
			}
		...
		case CG_CVAR_SET:
			...
			if (!strcmp((char *)arg[0], "cl_mac")){
				...
				arg[1] = (uintptr_t) &eth32.settings.jayMac[0];
				break;
			} else if (Tools.userCvarList) {
				cvarInfo_t *c = Tools.userCvarList;
				while (c) {
					if (!strcasecmp(c->name, (char *)arg[0])){
						arg[1] = (uintptr_t)c->ourValue;
						break;
					}
					c = c->next;
				}
...
{
	if (IS_WEAPATTRIB(lastweap, WA_SCOPED) && IS_WEAPATTRIB(newweap, WA_UNSCOPED) && !(Syscall.isKeyActionDown("+reload") || Syscall.isKeyActionDown("weapalt")))
		return;
...
{
	int i;
	const char *nullShader = "gfx/2d/nullshader.tga";
	...
	loadTga_s images[] = {
		// Name															// Replacement
		// Main menu/console logo
		{"ui/assets/et_logo_huge.tga", 									"gfx/2d/eth32_logo_huge.tga"},
		{"ui/assets/et_logo.tga", 										"gfx/2d/eth32_logo.tga"},
		// Hit particles
		{"gfx/misc/smokepuff.tga", 										nullShader},
		// Foliage
		{"models/mapobjects/plants_sd/grass_green1.tga", 				nullShader},
		{"models/mapobjects/plants_sd/grassfoliage1.tga", 				nullShader},
		{"models/mapobjects/plants_sd/bush_desert1.tga", 				nullShader},
		{"models/mapobjects/plants_sd/bush_desert2.tga", 				nullShader},
		{"models/mapobjects/plants_sd/deadbranch1.tga", 				nullShader},
		{"models/mapobjects/plants_sd/deadbranch2.tga", 				nullShader},
		{"models/mapobjects/plants_sd/deadbranch1_damp.tga", 			nullShader},
		{"models/mapobjects/plants_sd/deadbranch3.tga", 				nullShader},
		{"models/mapobjects/plants_sd/grass_dry3.tga", 					nullShader},
		{"models/mapobjects/plants_sd/catail2.tga", 					nullShader},
		{"models/mapobjects/plants_sd/catail1.tga", 					nullShader},
		{"models/mapobjects/tree_temperate_sd/leaves_temperate1.tga", 	nullShader},
		{"models/mapobjects/tree_temperate_sd/leaves_temperate2.tga", 	nullShader},
		{"models/mapobjects/tree_temperate_sd/leaves_temperate3.tga", 	nullShader},
		{"models/mapobjects/tree_desert_sd/palm_leaf1.tga", 			nullShader},
		{"models/mapobjects/trees_sd/winterbranch01.tga", 				nullShader},
		{"models/mapobjects/tree/branch_slp1.tga", 						nullShader},
		{"models/mapobjects/tree/branch_slp2.tga", 						nullShader},
	};

	// Main menu/console logo
	for (i=0; i<2; i++) {
		if (!strcmp(name, images[i].name)) {
			strcpy((char *)name, images[i].replacement);
			orig_LoadTGA(name, pic, width, height);

			// see if image failed to load (for example wrong/bad pk3)
			if (!*pic) {
				strcpy((char *)name, images[i].name);
				orig_LoadTGA(name, pic, width, height);
			}
		}
	}
	...
	if (eth32.settings.removeFoliage) {
		for (i=2; i<sizeof(images)/sizeof(images[0]); i++) {
			if (!strcmp(name, images[i].name)) {
				strcpy((char *)name, images[i].replacement);
				orig_LoadTGA(name, pic, width, height);
	...
	if (eth32.pb.statusOK) {
		...
		if (cv && eth32.pb.nCvars && *eth32.pb.nCvars) {
			for (int n=0; n<*eth32.pb.nCvars; n++) {
				...
				cv = (pb_cvar_t *)((uintptr_t)*eth32.pb.cvarlist+(n+1)*sizeof(pb_cvar_t));
			}
		}
	}
...
int CDECL eth32_CG_CalculateReinfTime(qboolean menu)
{
	...
	orig_CG_CalculateReinfTime = (CG_CalculateReinfTime_t) ((uintptr_t) eth32.cg.module + eth32.cgMod->CG_CalculateReinfTime);
...
void CDECL eth32_CG_EntityEvent(centity_t *cent, vec3_t position)
{
	...
	case EV_POPUPMESSAGE:
		if (es->effect1Time == PM_TEAM && es->density == 1) { // client disconnected
			int num = Tools.ClientNumForAddr((uintptr_t)cent);
